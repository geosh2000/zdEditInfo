...
export class ApiService {
  ...
  apiUrl = `${ Globals.APISERV }/ng2/json/`;
  apiRestful = `${ Globals.APISERV }/api/${Globals.APIFOLDER}/index.php/`;
  apiPostUrl = `${ Globals.APISERV }/ng2/post/`;
  qmAPI = `http://queuemetrics.pricetravel.com.mx:8080/queuemetricscc/`;
  ...
  restfulPut( params, apiRoute, loginReq = true ){
    ...
    if( loginReq ){
      let currentUser = JSON.parse(localStorage.getItem('currentUser'));
      url = `${ this.apiRestful }${ apiRoute }?token=${currentUser ? currentUser.token : 'noToken'}`
      ...
      url += `&usid=${currentUser ? currentUser.hcInfo.id : 'noId'}`
      url += `&zdId=${currentUser ? currentUser.hcInfo.zdId : 'noId'}&localIp=${this.localIp}`
    }else{
